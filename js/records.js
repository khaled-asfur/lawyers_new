...
session_actions="",
remind_time="";
var session_id_global =-1;
var row_index_global=-1;
var operation=" " ;// بحدد اذا بدي اضيف جلسة او اعرض جلسة

function fill_show_session_dialog(name,procecution_id ,session_number ,session_date ,remind_date ,remind_time ,session_id ,session_actions)
{
    ...
    document.getElementById('remind_time_update').value=remind_time;
    ...
    $('.actions button').click(function(){
      var idName= $(this).attr('id');
        ...
        if(idName=='view-button'){
            ...
            remind_time = $(row).find("td:nth-child(6)").find("input").val(),
            session_id = $(row).find("td:nth-child(7)").find("input").val(),
            session_actions = $(row).find("td:nth-child(8)").find("input").val();
            ...
            fill_show_session_dialog(name,procecution_id ,session_number ,session_date ,remind_date ,remind_time ,session_id ,session_actions);
          ...
          var div= document.getElementById('confirm-delete');
            ...
            row_index_global = row.index();
/*
            var td_1 = $(row).find("td:nth-child(1)").find("input"),
            td_2= $(row).find("td:nth-child(2)").find("input"), 
            td_3=$(row).find("td:nth-child(3)").find("input"),
            td_4 = $(row).find("td:nth-child(4)").find("input"),
            td_5 = $(row).find("td:nth-child(5)").find("input");*/
        }
    });
  ...
  $("#confirm-yes-delete").click(function(){
    ...
    $("#confirm-yes-edit").click(function(){
       ...
       var  row=(document.getElementById("my_table").rows[row_index_global+1]);
        ...
        update_Session(session_number,
            session_date,remind_date,remind_time,session_actions ,session_id)
     ...
     $("#addNew").click(function(){
      ...
      document.getElementById('session_number').value="";
        ...
        document.getElementById('remind_time').value="";
    ...
    $("#save_update_dialog").click(function(){
        ...
        var remind_time= document.getElementById('remind_time_update').value;
        var session_actions =  document.getElementById('actions_update').value;

        update_Session(session_number,session_date,remind_date,remind_time,session_actions ,session_id_global);
          });    
        ...
        $("#save").click(function(){
            ...
            var div= document.getElementById('add_session_dialog');
            $(div).css('display','none');
          var name= document.getElementById('search_name_id_no_input_dialog').value;
          ...
          var remind_time= document.getElementById('remind_time').value;
          ...
          //تحديد اذا كانت العملية اضافة جلسة جديدة او مشاهدة جلسة وحفظ التعديلات 
             ...
             insert_Session(  procecution_number ,session_number,session_date,remind_date,remind_time,session_actions);
        ...
        function update_Session(session_number,
            ...
            "&remind_date="+remind_date+"&remind_time="+remind_time+"&actions="+session_actions+"&session_id="+session_id+"&operation="+ operation,true);
            xmlhttp.send();    
        }
